...
def fit_pot(scores: np.ndarray, risk: float = 1e-4, level: float = 0.98) -> Result[float]:
    """
    ...
    Returns:
        Result[float]: Ok(threshold) on success (including fallback), Err on critical failure.
    """
    # Ensure numpy array
    ...
    if num_peaks < 10:
        ...
        return Ok(np.max(scores) * 1.1)

    # 3. Fit GPD
    # genpareto.fit returns (shape, loc, scale)
    # We fix loc=0 because we are fitting the excesses
    # FIXME: Exceptions during fitting (e.g. convergence) will bubble up
    ...
    logger.info(
        f"POT Threshold calculated: {threshold:.6f} (Initial t: {t:.6f}, Peaks: {num_peaks}, Shape: {c:.4f}, Scale: {scale:.4f})")
    return Ok(threshold)
